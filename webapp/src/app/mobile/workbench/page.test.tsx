...
import MobileWorkbenchPage from "./page";

const PAGE_FILE = resolve(process.cwd(), "src/app/mobile/workbench/page.tsx");
const DOMAIN_IMPORT = "@/features/mobile/workbench/MobileWorkbenchGate";
...
function assertExactPageBoundary(source: string): void {
  ...
  expect(
    imports[0].importClause?.namedBindings &&
      ...
      : [],
  ).toEqual(["MobileWorkbenchGate"]);
  expect(jsxTags).toEqual(["MobileWorkbenchGate"]);
  expect(dynamicImports).toHaveLength(0);
  expect(source).not.toMatch(/^\s*["']use client["'];/m);
}

describe("Mobile Workbench page", () => {
  it("renders exactly one route heading and the Workbench boundary marker", () => {
    const html = renderToStaticMarkup(<MobileWorkbenchPage />);
    const host = document.createElement("div");
    host.innerHTML = html;

    expect(host.querySelectorAll("h1")).toHaveLength(1);
    expect(host.textContent).toContain("WORKBENCH / CAPABILITY BOUNDARY");
  });
