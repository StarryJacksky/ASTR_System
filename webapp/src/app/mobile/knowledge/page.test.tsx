...
import MobileKnowledgePage from "./page";

const PAGE_FILE = resolve(process.cwd(), "src/app/mobile/knowledge/page.tsx");
const DOMAIN_IMPORT = "@/features/mobile/knowledge/MobileKnowledgeGate";
...
function assertExactPageBoundary(source: string): void {
  ...
  expect(
    imports[0].importClause?.namedBindings &&
      ...
      : [],
  ).toEqual(["MobileKnowledgeGate"]);
  expect(jsxTags).toEqual(["MobileKnowledgeGate"]);
  expect(dynamicImports).toHaveLength(0);
  expect(source).not.toMatch(/^\s*["']use client["'];/m);
}

describe("Mobile Knowledge page", () => {
  it("renders exactly one route heading and the Knowledge boundary marker", () => {
    const html = renderToStaticMarkup(<MobileKnowledgePage />);
    const host = document.createElement("div");
    host.innerHTML = html;

    expect(host.querySelectorAll("h1")).toHaveLength(1);
    expect(host.textContent).toContain("KNOWLEDGE / READ PROJECTION");
  });
