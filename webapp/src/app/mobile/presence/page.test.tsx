...
import MobilePresencePage from "./page";

const PAGE_FILE = resolve(process.cwd(), "src/app/mobile/presence/page.tsx");
const DOMAIN_IMPORT = "@/features/mobile/presence/MobilePresenceDomain";
...
function assertExactPageBoundary(source: string): void {
  ...
  expect(
    imports[0].importClause?.namedBindings &&
      ...
      : [],
  ).toEqual(["MobilePresenceDomain"]);
  expect(jsxTags).toEqual(["MobilePresenceDomain"]);
  expect(dynamicImports).toHaveLength(0);
  expect(source).not.toMatch(/^\s*["']use client["'];/m);
}

describe("Mobile Presence page", () => {
  it("renders exactly one route heading and the Presence authority marker", () => {
