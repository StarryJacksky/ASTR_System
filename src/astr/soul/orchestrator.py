...
from astr.soul import emotion, fewshot, life, moa
...
class SoulOrchestrator:
    ...
    async def respond(
        ...
        *,
        ...
        if speaker:
            try:
                async with self._state_lock:  # 并行下保护 people.json 读改写
        ...
        # 发言习惯锚定（金标 few-shot）：照主人定义的腔说话，别把设定背成 AI 腔（治"不像人"）
        shots = fewshot.sample_turns(6)
        if shots:
            messages.append(
                {
                    "role": "system",
                    "content": "下面几轮是你（秋秋）平时真实的说话风格，照这股劲儿回——短、有态度、"
                    "不解释自己、不端着、不背设定。这是范例不是要你照抄内容：",
                }
            )
            messages += shots
        ...
        if recent:
            ...
            messages.append(
                {
                    "role": "system",
                    "content": f"（{'群里' if is_group else ''}最近在聊这些，接话要顺着语境、别答非所问）：\n{tail}",
                }
            )
        messages += [
        ...
        async with self._state_lock:
            fresh_mood = emotion.apply(emotion.decayed(emotion.load(self.soul_name)), delta)
            emotion.save(fresh_mood, self.soul_name)
        report["emotion_delta"] = delta
        report["emotion_state"] = fresh_mood.model_dump(mode="json")
